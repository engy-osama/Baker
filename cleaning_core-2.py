...
# Matches "<base> - <branch>", e.g. "المنطقه - القاهره" -> base="المنطقه".
# Forms clones a branching question once per choice of its parent question,
# naming each clone "<original question text> - <parent answer text>" (as
# opposed to the numeric-suffix style "المنطقه", "المنطقه2", ... it also
# uses elsewhere). Both patterns mean the same thing: only one of the group
# is ever filled in per row, and they should be merged into a single column.
_DASH_BRANCH_RE = re.compile(r'^(.*\S)\s*-\s*(\S.*)$')


def detect_dash_branching_groups(df):
    """Return {base: [col, col, ...]} for headers sharing an identical
    "<base> - " prefix, when 2+ columns share that exact base."""
    groups = defaultdict(list)
    order = []
    for col in df.columns:
        m = _DASH_BRANCH_RE.match(str(col))
        if not m:
            continue
...
def clean_dataframe(df, dictionaries=None, drop_empty_cols=True, drop_columns=DEFAULT_DROP_COLUMNS):
    ...
    # 1. Merge Forms' branching-question column groups -- both the
    #    numbered-suffix style (المنطقه, المنطقه2, ...) and the dash-suffix,
    #    per-choice style (المنطقه - القاهره, المنطقه - الجيزه, ...) -- into
    #    a single column each, before any translation happens.
    merge_info = []

    def _merge_groups(cleaned, groups):
    ...
    cleaned = _merge_groups(cleaned, detect_branching_groups(cleaned))
    cleaned = _merge_groups(cleaned, detect_dash_branching_groups(cleaned))
